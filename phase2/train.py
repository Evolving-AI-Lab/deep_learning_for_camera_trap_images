...
def exclude():
  """
     Removes variables of the output layer when loading a snapshot from disk for transfer learning
  """
...
def loss(logits, labels, output_names):
  """
  ...
  """
  cross_entropy_losses=[None]*len(logits)
  for i in xrange(0,len(logits)):
    cross_entropy_losses[i] =tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels= labels[:,i], logits= logits[i]))
    tf.summary.scalar('Cross Entropy Loss ['+output_names[i]+']', cross_entropy_losses[i])
...
def train(args):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    output_names=['Identification','Count','Standing','Resting','Moving','Eating','Interacting','Babies']
          ...
          with tf.name_scope('Tower_%d' % i) as scope:
            ...
            for i in xrange(0,len(logits)):
              top1acc[i] = tf.reduce_mean(
                      tf.cast(tf.nn.in_top_k(logits[i], labels[:,i], 1), tf.float32))
              tf.summary.scalar('Top-1 Accuracy ['+output_names[i]+']', top1acc[i])
            # Top-5 ID accuracy
            top5acc_id= tf.reduce_mean(
                      tf.cast(tf.nn.in_top_k(logits[0], labels[:,0], 5), tf.float32))
            # Top-3 count accuracy
            top3acc_count= tf.reduce_mean(
                      tf.cast(tf.nn.in_top_k(logits[1], labels[:,1], 3), tf.float32)) 
 
            tf.summary.scalar('Top-5 Accuracy [Identification]', top5acc_id)
            tf.summary.scalar('Top-3 Accuracy [Count]', top3acc_count)
            # Build the portion of the Graph calculating the losses. Note that we will
            # assemble the total_loss using a custom function below.
            cross_entropy_mean = loss(logits, labels, output_names)
            ...
            total_loss = tf.add(tf.add_n(cross_entropy_mean), reg_loss)
            ...
            if args.transfer_mode[0]== 3:
              ...
              aux_grads = opt.compute_gradients(total_loss)
    ...
    if args.transfer_mode[0]==3:
      ...
      train_op = tf.cond(tf.less(epoch_number,args.transfer_mode[1]), lambda: tf.group(opt.apply_gradients(grads), batchnorm_updates_op),
              lambda: tf.group(opt.apply_gradients(aux_grads), batchnorm_updates_op))
    elif args.transfer_mode[0]==1:
    ...
    for epoch in xrange(start_epoch, start_epoch + args.num_epochs):
      ...
      for step in xrange(args.num_batches):
        ...
        _, loss_value, top1_accuracy, top5_accuracy_id, top3_accuracy_count = sess.run(
            [train_op, cross_entropy_mean, top1acc, top5acc_id, top3acc_count], options= run_options, run_metadata= run_metadata)
        duration = time.time() - start_time

        # Check for errors
        assert not np.any(np.isnan(loss_value)), 'Model diverged with loss = NaN'
        ...
        if step % 10 == 0:
          ...
          format_str = ('%s: epoch %d, step %d, loss = %s, Top-1 = %s, Top-5 Id= %.2f, Top-3 Count= %.2f (%.1f examples/sec; %.3f '
                        'sec/batch)')
          print (format_str % (datetime.now(), epoch, step, '[' + ', '.join('%.3f' % item for item in loss_value) + ']',
              '[' + ', '.join('%.3f' % item for item in top1_accuracy) + ']', top5_accuracy_id, top3_accuracy_count,
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--data_info', default= 'silver_info_train.csv', action= 'store', help= 'Name of the file containing addresses and labels of training images')
    ...
    parser.add_argument('--depth', default= 50, type= int, action= 'store', help= 'The depth of ResNet or DenseNet architectures')
    ...
    parser.add_argument('--transfer_mode', default = [0], nargs='+', type= int, help= 'Transfer mode 0=None , 1=Tune last layer only , 2= Tune all the layers')
