...
def inference(x, num_output, wd, dropout_rate, is_training, transfer_mode= False):
  ...
  with tf.variable_scope('mainb'):
    ...
    main_output = [None]*len(num_output)
    with tf.variable_scope('output'):
      for o in xrange(0,len(num_output)):
        with tf.variable_scope('moutput'+str(o)):
  ...
  with tf.variable_scope('auxb'):
    ...
    aux_output = [None]*len(num_output)
    with tf.variable_scope('output'):
      for o in xrange(0,len(num_output)):
        with tf.variable_scope('aoutput'+str(o)):
          aux_output[o] = common.fullyConnected(network, num_output[o],  wd= wd)

  output=[None]*len(num_output)
  for i in xrange(0,len(num_output)):
    output[i]= tf.concat([main_output[i], aux_output[i]], 1)
